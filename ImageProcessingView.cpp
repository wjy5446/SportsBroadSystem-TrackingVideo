...
void CImageProcessingView::OnDraw(CDC* pDC)
{
	...
	// Result 출력
	// 필요한 비트맵 해더 정보 수정
	m_BmiHeader.biWidth = pDoc->m_Result.nW;
	m_BmiHeader.biHeight = pDoc->m_Result.nH;
	m_BmiHeader.biSizeImage = GetBmp24Size(pDoc->m_Result.nW, pDoc->m_Result.nH);

	// 영상 출력
	StretchDIBits(pDC->m_hDC,
		pDoc->m_Result.nPosX, pDoc->m_Result.nPosY,
		m_BmiHeader.biWidth,
		m_BmiHeader.biHeight,
		0, 0,
		m_BmiHeader.biWidth,
		m_BmiHeader.biHeight,
		pDoc->m_Result.Image1D,
		(LPBITMAPINFO)&m_BmiHeader,
		DIB_RGB_COLORS,
		SRCCOPY);
	...
	if (m_bActiveZ1 == true){
		// 필요한 비트맵 해더 정보 수정
		m_BmiHeader.biWidth = pDoc->m_Zoom1.nW;
		m_BmiHeader.biHeight = pDoc->m_Zoom1.nH;
		m_BmiHeader.biSizeImage = GetBmp24Size(pDoc->m_Zoom1.nW, pDoc->m_Zoom1.nH);

		// 영상 출력
		StretchDIBits(pDC->m_hDC,
			pDoc->m_Zoom1.nPosX, pDoc->m_Zoom1.nPosY,
			m_BmiHeader.biWidth,
			m_BmiHeader.biHeight,
			0, 0,
			m_BmiHeader.biWidth,
			m_BmiHeader.biHeight,
			pDoc->m_Zoom1.Image1D,
			(LPBITMAPINFO)&m_BmiHeader,
			DIB_RGB_COLORS,
			SRCCOPY);
	}
	...
	if (m_bActiveZ2 == true){
		// 필요한 비트맵 해더 정보 수정
		m_BmiHeader.biWidth = pDoc->m_Zoom2.nW;
		m_BmiHeader.biHeight = pDoc->m_Zoom2.nH;
		m_BmiHeader.biSizeImage = GetBmp24Size(pDoc->m_Zoom2.nW, pDoc->m_Zoom2.nH);

		// 영상 출력
		StretchDIBits(pDC->m_hDC,
			pDoc->m_Zoom2.nPosX, pDoc->m_Zoom2.nPosY,
			m_BmiHeader.biWidth,
			m_BmiHeader.biHeight,
			0, 0,
			m_BmiHeader.biWidth,
			m_BmiHeader.biHeight,
			pDoc->m_Zoom2.Image1D,
			(LPBITMAPINFO)&m_BmiHeader,
			DIB_RGB_COLORS,
			SRCCOPY);
	}
	...
	if (m_bActiveZ3 == true){
		// 필요한 비트맵 해더 정보 수정
		m_BmiHeader.biWidth = pDoc->m_Zoom3.nW;
		m_BmiHeader.biHeight = pDoc->m_Zoom3.nH;
		m_BmiHeader.biSizeImage = GetBmp24Size(pDoc->m_Zoom3.nW, pDoc->m_Zoom3.nH);

		// 영상 출력
		StretchDIBits(pDC->m_hDC,
			pDoc->m_Zoom3.nPosX, pDoc->m_Zoom3.nPosY,
			m_BmiHeader.biWidth,
			m_BmiHeader.biHeight,
			0, 0,
			m_BmiHeader.biWidth,
			m_BmiHeader.biHeight,
			pDoc->m_Zoom3.Image1D,
			(LPBITMAPINFO)&m_BmiHeader,
			DIB_RGB_COLORS,
			SRCCOPY);
	}
...
// Result 영상 내부에 어떤 타겟을 선택했는지 확인
int  CImageProcessingView::CheckTargetInResult(CPoint point, int area){
	CImageProcessingDoc* pDoc = GetDocument();
	ASSERT_VALID(pDoc);
	...
	if (pDoc->m_bIsZoom1)
	{
		CRect rt1(pDoc->m_Result.nPosX + pDoc->m_ptTarget1.x - area, pDoc->m_Result.nPosY + pDoc->m_ptTarget1.y - area,
			pDoc->m_Result.nPosX + pDoc->m_ptTarget1.x + area, pDoc->m_Result.nPosY + pDoc->m_ptTarget1.y + area);

		// 사각형에 point 포함 여부 확인
		if (rt1.PtInRect(CurrentPoint))
			return 1;
	}

	if (pDoc->m_bIsZoom2)
	{
		CRect rt2(pDoc->m_Result.nPosX + pDoc->m_ptTarget2.x - area, pDoc->m_Result.nPosY + pDoc->m_ptTarget2.y - area,
			pDoc->m_Result.nPosX + pDoc->m_ptTarget2.x + area, pDoc->m_Result.nPosY + pDoc->m_ptTarget2.y + area);

		// 사각형에 point 포함 여부 확인
		if (rt2.PtInRect(CurrentPoint))
			return 2;
	}

	if (pDoc->m_bIsZoom3)
	{
		CRect rt3(pDoc->m_Result.nPosX + pDoc->m_ptTarget3.x - area, pDoc->m_Result.nPosY + pDoc->m_ptTarget3.y - area,
			pDoc->m_Result.nPosX + pDoc->m_ptTarget3.x + area, pDoc->m_Result.nPosY + pDoc->m_ptTarget3.y + area);

		// 사각형에 point 포함 여부 확인
		if (rt3.PtInRect(CurrentPoint))
			return 3;
	}
...
bool CImageProcessingView::AddResultInDoc(int nW, int nH, int nPosX, int nPosY, BYTE** ImageR, BYTE** ImageG, BYTE** ImageB, CPoint target1, CPoint target2, CPoint target3, int frame)
{
	CImageProcessingDoc* pDoc = GetDocument();
	ASSERT_VALID(pDoc);
	...
	pDoc->m_Result.nW = nW;
	pDoc->m_Result.nH = nH;
	pDoc->m_Result.nPosX = nPosX;
	pDoc->m_Result.nPosY = nPosY;

	if (frame == 0)
		pDoc->m_Result.Image1D = new BYTE[GetBmp24Size(nW, nH)];
	...
	for (y = 0; y < nH; y++)
		for (x = 0; x < nW; x++)
		{
			...
			pDoc->m_Result.Image1D[Pos++] = ImageB[y][x];
			pDoc->m_Result.Image1D[Pos++] = ImageG[y][x];
			pDoc->m_Result.Image1D[Pos] = ImageR[y][x];
		}

	pDoc->m_bIsResult = true;

	
	if (target1.y >= 0 && target1.x >= 0)
		pDoc->m_ptTarget1 = target1;
	if (target2.y >= 0 && target2.x >= 0)
		pDoc->m_ptTarget2 = target2;
	if (target3.y >= 0 && target3.x >= 0)
		pDoc->m_ptTarget3 = target3;


	return true;
}

bool CImageProcessingView::AddZoom1InDoc(int nW, int nH, int nPosX, int nPosY, BYTE* ImageColor, int frame)
{
	CImageProcessingDoc* pDoc = GetDocument();
	ASSERT_VALID(pDoc);
	...
	pDoc->m_Zoom1.nW = nW;
	pDoc->m_Zoom1.nH = nH;
	pDoc->m_Zoom1.nPosX = nPosX;
	pDoc->m_Zoom1.nPosY = nPosY;
	
	if (frame == 0)
		pDoc->m_Zoom1.Image1D = new BYTE[GetBmp24Size(nW, nH)];
	
	memcpy(pDoc->m_Zoom1.Image1D, ImageColor, GetBmp24Size(nW, nH));

	pDoc->m_bIsZoom1 = true;
	
	return true;
}

bool CImageProcessingView::AddZoom2InDoc(int nW, int nH, int nPosX, int nPosY, BYTE* ImageColor, int frame)
{
	CImageProcessingDoc* pDoc = GetDocument();
	ASSERT_VALID(pDoc);
	...
	pDoc->m_Zoom2.nW = nW;
	pDoc->m_Zoom2.nH = nH;
	pDoc->m_Zoom2.nPosX = nPosX;
	pDoc->m_Zoom2.nPosY = nPosY;

	if (frame == 0)
		pDoc->m_Zoom2.Image1D = new BYTE[GetBmp24Size(nW, nH)];

	memcpy(pDoc->m_Zoom2.Image1D, ImageColor, GetBmp24Size(nW, nH));

	pDoc->m_bIsZoom2 = true;

	return true;
}

bool CImageProcessingView::AddZoom3InDoc(int nW, int nH, int nPosX, int nPosY, BYTE* ImageColor, int frame)
{
	CImageProcessingDoc* pDoc = GetDocument();
	ASSERT_VALID(pDoc);
	...
	pDoc->m_Zoom3.nW = nW;
	pDoc->m_Zoom3.nH = nH;
	pDoc->m_Zoom3.nPosX = nPosX;
	pDoc->m_Zoom3.nPosY = nPosY;

	if (frame == 0)
		pDoc->m_Zoom3.Image1D = new BYTE[GetBmp24Size(nW, nH)];
	
	memcpy(pDoc->m_Zoom3.Image1D, ImageColor, GetBmp24Size(nW, nH));

	pDoc->m_bIsZoom3 = true;
...
{
	int nCurrentTarget;
	// Result영상 내부 타켓 정보 가져오기
	nCurrentTarget = CheckTargetInResult(point, 10);

	if (nCurrentTarget == 1){// Z1일 때 
		if (m_bActiveZ1 == false){
			m_bActiveZ1 = true;
			m_bActiveZ2 = false;
			m_bActiveZ3 = false;
			...
			//if (!pMain->m_pZoom1->m_bCreated)
			//{
			//	pMain->m_pZoom1->Create(CZoom1Dlg::IDD, pMain);
			//	pMain->m_pZoom1->ShowWindow(SW_SHOW);

			//	pMain->m_pZoom1->m_bCreated = true;
			//}
		}
		else{
			m_bActiveZ1 = false;
			...
			/*CMainFrame *pMain = (CMainFrame *)AfxGetMainWnd();
			pMain->m_pZoom1->DestroyWindow();
			pMain->m_pZoom1->m_bCreated = false;*/
		}
	}

	if (nCurrentTarget == 2){// Z1일 때 
		if (m_bActiveZ2 == false){
			m_bActiveZ2 = true;
			m_bActiveZ1 = false;
			m_bActiveZ3 = false;
		}
	...
	if (nCurrentTarget == 3){// Z1일 때 
		if (m_bActiveZ3 == false){
			m_bActiveZ3 = true;
			m_bActiveZ1 = false;
			m_bActiveZ2 = false;
		}
	...
	else if(m_nCurrentImage > -1)
	{
		...
		m_PrevRect = CRect(0, 0, 0, 0);
		SetCapture();	// 마우스 캡쳐
		// 마우스 커서가 윈도우 영역을 벗어나도 마우스 메세지를 받음
		// 상황 해제시 반드시 ReleaseCapture 함수 사용
	}
...
void CImageProcessingView::GetCurrPosColor1D(int x, int y, BYTE *Image1D)
{
	...
	if (m_bActiveZ1 == true){
		if (pDoc->m_Zoom1.nPosX <= x &&
			pDoc->m_Zoom1.nPosX + pDoc->m_Zoom1.nW > x &&
			pDoc->m_Zoom1.nPosY <= y &&
			pDoc->m_Zoom1.nPosY + pDoc->m_Zoom1.nH > y)
		{
			offset = GetBmp24Pos(pDoc->m_Zoom1.nW, pDoc->m_Zoom1.nH, x - pDoc->m_Zoom1.nPosX, y - pDoc->m_Zoom1.nPosY);

			*Image1D = pDoc->m_Zoom1.Image1D[offset];
			*(Image1D + 1) = pDoc->m_Zoom1.Image1D[offset + 1];
			*(Image1D + 2) = pDoc->m_Zoom1.Image1D[offset + 2];
	...
	if (m_bActiveZ2 == true){
		if (pDoc->m_Zoom2.nPosX <= x &&
			pDoc->m_Zoom2.nPosX + pDoc->m_Zoom2.nW > x &&
			pDoc->m_Zoom2.nPosY <= y &&
			pDoc->m_Zoom2.nPosY + pDoc->m_Zoom2.nH > y)
		{
			offset = GetBmp24Pos(pDoc->m_Zoom2.nW, pDoc->m_Zoom2.nH, x - pDoc->m_Zoom2.nPosX, y - pDoc->m_Zoom2.nPosY);

			*Image1D = pDoc->m_Zoom2.Image1D[offset];
			*(Image1D + 1) = pDoc->m_Zoom2.Image1D[offset + 1];
			*(Image1D + 2) = pDoc->m_Zoom2.Image1D[offset + 2];
	...
	if (m_bActiveZ3 == true){
		if (pDoc->m_Zoom3.nPosX <= x &&
			pDoc->m_Zoom3.nPosX + pDoc->m_Zoom3.nW > x &&
			pDoc->m_Zoom3.nPosY <= y &&
			pDoc->m_Zoom3.nPosY + pDoc->m_Zoom3.nH > y)
		{
			offset = GetBmp24Pos(pDoc->m_Zoom3.nW, pDoc->m_Zoom3.nH, x - pDoc->m_Zoom3.nPosX, y - pDoc->m_Zoom3.nPosY);

			*Image1D = pDoc->m_Zoom3.Image1D[offset];
			*(Image1D + 1) = pDoc->m_Zoom3.Image1D[offset + 1];
			*(Image1D + 2) = pDoc->m_Zoom3.Image1D[offset + 2];
